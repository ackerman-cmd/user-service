...
class UserControllerV1IntegrationTest : AbstractIntegrationTest() {
    ...
    @Test
    fun `change user status returns 200`() {
        val registered = authService.register(TestUtils.createRegisterUserCommand(username = "user-status"))

        mockMvc
            .patch("/api/v1/users/{id}/status", registered.id) {
